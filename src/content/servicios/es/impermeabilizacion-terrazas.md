---
title: "Impermeabilizar terraza en Madrid — 25-45 €/m²"
metaDescription: "Impermeabilización de terrazas y cubiertas en Madrid con membrana líquida o tela asfáltica: 25-45 €/m² y garantía por escrito. Adiós a las goteras."
h1: "Impermeabilización de terrazas en Madrid"
translationKey: service-terrazas
locale: es
serviceName: "Impermeabilización de terrazas"
priceKey: impermeabilizacion
shortLabel: "Impermeabilizar terrazas"
excerpt: "Diagnóstico del origen real de la filtración, sistema con refuerzo en puntos críticos y revisión al año."
orden: 7
directAnswer: "Impermeabilizar una terraza en Madrid cuesta entre 25 y 45 €/m² en 2026 con membrana de poliuretano líquido, incluida la preparación del soporte. Una terraza de 20 m² queda entre 500 € y 900 € en 2-3 días, con garantía por escrito de 10 años en el sistema aplicado."
datePublished: 2026-07-09
dateModified: 2026-07-09
relatedServices:
  - es/pintura-fachadas-comunidades
  - es/pintura-interior-pisos
proceso:
  - paso: "Diagnóstico de la filtración"
    detalle: "Localizamos el origen real de la humedad — no siempre está donde aparece la mancha — y comprobamos pendientes y desagües."
  - paso: "Preparación del soporte"
    detalle: "Limpieza, reparación de fisuras, tratamiento de juntas y puntos singulares (sumideros, encuentros con paramentos)."
  - paso: "Sistema con refuerzo"
    detalle: "Membrana de poliuretano líquido armada con malla en los puntos críticos, aplicada en dos capas cruzadas."
  - paso: "Entrega con garantía"
    detalle: "Acabado transitable del color que elijas, garantía por escrito y revisión gratuita al año."
faq:
  - q: "¿Cómo sabéis dónde está realmente la filtración?"
    a: "Inspeccionando antes de presupuestar: la mancha del techo casi nunca está justo debajo del fallo, porque el agua viaja por la estructura. Comprobamos pendientes, desagües, juntas y encuentros antes de proponer nada — tratar el punto equivocado es dinero tirado, aunque el trabajo esté bien hecho."
  - q: "¿Por qué falla la mayoría de las impermeabilizaciones?"
    a: "En los detalles, no en la superficie: sumideros, esquinas y encuentros con muros. Por eso armamos la membrana con malla de refuerzo exactamente en esos puntos. Es la diferencia entre un sistema que dura 3 años y uno que dura 15."
  - q: "¿Se puede impermeabilizar sobre las baldosas existentes?"
    a: "Sí, si el embaldosado está firme: la membrana líquida se aplica directamente sobre baldosa con imprimación previa, evitando la obra de levantar el suelo — menos coste, menos escombro y menos días."
  - q: "¿Qué garantía tengo de que no vuelva la gotera?"
    a: "Garantía por escrito del sistema aplicado, la ficha técnica del fabricante y una revisión gratuita al año para comprobar que todo sigue perfecto. Si algo fallara en el periodo de garantía, lo reparamos nosotros sin discusiones."
  - q: "¿Trabajáis cubiertas de comunidades?"
    a: "Sí, tanto terrazas privadas como cubiertas comunitarias, con presupuesto por partidas para la junta y coordinación con el administrador. Si el edificio va a montar andamio para la fachada, es el momento óptimo de tratar también la cubierta."
  - q: "¿Cuánto cuesta impermeabilizar una terraza?"
    a: "Entre 25 y 45 €/m² en 2026 con membrana de poliuretano líquido y preparación incluida; una terraza de 20 m², entre 500 y 900 €. El presupuesto exacto sale de la inspección gratuita."
---

## ¿Por qué el diagnóstico importa más que el material?

Porque el agua engaña. La mancha del dormitorio puede venir de una junta a cuatro metros, de un sumidero mal sellado o de una pendiente que embalsa — y una impermeabilización perfecta aplicada en el sitio equivocado no arregla nada. Por eso nuestra visita no es un vistazo para dar precio: comprobamos pendientes, desagües, juntas y encuentros, y localizamos el origen real antes de proponer solución. A veces la respuesta honesta es una reparación puntual de 200 € y no un sistema completo; también te lo diremos.

## ¿Qué hace que un sistema dure 15 años en el clima de Madrid?

Madrid castiga las cubiertas por los dos extremos: ciclos de hielo-deshielo en invierno y superficies a más de 70 °C en verano. Un sistema duradero necesita tres cosas: membrana continua sin juntas ni solapes (por eso trabajamos con poliuretano líquido en dos capas cruzadas), **refuerzo con malla en los puntos singulares** — sumideros, esquinas, encuentros con muros, que es donde fallan casi todas —, y una preparación del soporte sin atajos. El acabado queda transitable y del color que elijas, con la ficha técnica del sistema entregada junto a la garantía.

## ¿Qué pasa después de la obra?

No desaparecemos: garantía por escrito del sistema aplicado y una revisión gratuita al año, porque una cubierta se comporta distinto tras pasar su primer verano y su primer invierno. Si tu comunidad está planificando obra de fachada, coordina ambas: con el andamio ya montado, tratar la cubierta cuesta sensiblemente menos — lo gestionamos en un mismo presupuesto por partidas junto a la [pintura de fachadas](/servicios/pintura-fachadas-comunidades/). Los rangos orientativos (25-45 €/m² en 2026) están publicados para que hagas números antes de llamar.
