...
- **Zaraz API gotchas learned (do not relearn):** tool purpose field is `defaultPurpose` (+ requires `vendorName`/`vendorPolicyUrl` on Custom HTML); consent needs the plain `purposes` map, not just `purposesWithTranslations`; auto-inject DOES inject for browser user-agents (curl showed nothing and misled us — 2026-07-13) so with the manual `<script src>` in BaseLayout it double-loads ("zaraz is loaded twice"); `settings.autoInjectScript` is now **false** — keep it false, the manual tag is the only loader; Google Ads gtag runs OUTSIDE Zaraz with Consent Mode v2 defaults denied, synced from Zaraz consent (`zarazConsentAPIReady`/`zarazConsentChoicesUpdated` → purposes `marketing`/`analitica`) in BaseLayout; `/cdn-cgi/zaraz/*` is ad-block-listed → custom paths `settings.initPath=/al/i.js, scriptPath=/al/s.js, trackPath=/al/t, mcRootPath=/al/mc`; zone workflow is `realtime` (PUTs are live instantly, no publish).
