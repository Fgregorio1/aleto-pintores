...
- **Modal language = PAGE language** (product decision 2026-07-12): Zaraz natively resolves language from browser Accept-Language at the edge (es default when unknown) with no page-level override, so `Attribution.astro` rewrites the modal strings in the open shadow root to match `document.documentElement.lang`. ⚠️ The string table in Attribution.astro MUST stay in sync with the Zaraz config texts. Zaraz's native title ("Cookie Settings") is hardcoded EN for all languages → workaround: `.title_container` hidden via CSS, translated title embedded as `<span class="al-title">` inside `consentModalIntroHTMLWithTranslations`.
